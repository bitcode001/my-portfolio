...
function SkillBadge({ activeLang, myIndex, clickHandler, children }: SkillBadgeInterface) {
	return (
		...
		>
			<div
				className={cn(
					'absolute flex h-full w-full scale-0 rounded-full transition-all duration-300',
					{
						'scale-100 animate-pulse-badge bg-primary duration-1000': activeLang === myIndex
					}
				)}
			/>
...
export default function SkillsetSectionOld() {
	...
	return (
		<section className="relative overflow-x-clip bg-indigo-50 py-10" id='skillset'>
			{/* Animating Circle */}
			{/* <div className="absolute -right-2 top-10 flex scale-50 items-center justify-center opacity-50 md:-right-4 md:scale-100 md:opacity-100">
				<div
					className="absolute z-20 h-28 w-28 animate-spin-slow rounded-full bg-transparent"
					style={{
				...
				/>
				<div
					className="absolute z-20 h-60 w-60 animate-spin-slow-reverse rounded-full bg-transparent"
					style={{
				...
				/>
				<div
					className="absolute z-20 animate-spin-slow-x2 rounded-full bg-transparent"
					style={{
				...
				<div className="flex flex-row justify-between md:flex-col">
					...
					<SkillBadge myIndex={2} activeLang={activeLang} clickHandler={setActiveLang}>
						...
						>
							<path
								...
								fill={activeLang === 2 ? '#ffffff' : '#5769F8'}
							/>
							<path
								className={svgColorTransition}
					...
					<SkillBadge myIndex={3} activeLang={activeLang} clickHandler={setActiveLang}>
						...
						>
							...
							/>
							<g className={svgColorTransition} fill={activeLang === 3 ? '#5769F8' : '#ffffff'}>
								<path
				...
				<div className="flex w-full flex-col p-8 md:p-20">
					...
					{mySkillSet.map((sk, idx) => (
						<div
							...
							key={idx}
						>
							<h4 className="my-10 animate-slide-in text-xl font-medium">{sk.name}</h4>
							<p
